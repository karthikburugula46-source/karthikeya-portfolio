...
- **Ambient motion, redesigned smooth**: `spawnSparkle()` (index.html, near `setMode`) drifts one small flat-color pixel upward and fades (`ease-out`, ~2.1s) every 1.4–3.2s while idle in gamer mode — replaces the old stepped glitch-bar flicker. `modePowerFlash()` replaces the old 5-bar glitch burst with one calm full-viewport opacity pulse (`ease-out`, ~400ms) on every PM↔Gamer toggle.
